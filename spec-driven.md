...
Spec-Driven Development can be entered at different stages depending on how much you already know about your project. Spec Kit supports two primary paths:

| Path | Best When | Entry Command |
|------|-----------|---------------|
| **Feature-First** | You have an existing project with a constitution already defined, and you're adding discrete features | `/speckit.specify` |
| **Brainstorm-First** | You're starting a new product from scratch, or requirements are unclear and need collaborative discovery | `/speckit.brainstorm` |

---

## The Brainstorm-First Workflow

When starting a **greenfield project** or when requirements are vague, the brainstorm-first workflow helps you discover, design, and codify your product before writing a single line of code. The four-step chain produces all the context needed to then use the feature-first workflow.

```text
speckit.brainstorm → speckit.uidesign → speckit.initiate → speckit.specify
        ↓                    ↓                  ↓                 ↓
  product-spec.md       ui-spec.md        constitution.md      spec.md
```

### Step 1: `/speckit.brainstorm` — Product Discovery

An interactive Q&A session guides you through key product decisions:

- **Product nature** — what problem it solves, who uses it
- **Platform & deployment** — web, mobile, desktop, API, cloud region
- **Architecture style** — monolith, microservices, serverless
- **Authentication model** — public, auth, roles, OAuth providers
- **Scale expectations** — users, data volume, SLAs
- **Core feature set** — the MVP vs. future backlog

The session ends with a fully structured `product-spec.md` saved to `.specify/memory/`. This document captures technology recommendations, architecture decisions, and functional requirements—the seed for everything that follows.

**Use this command when:**
- Starting a new product or service from scratch
- Evaluating different architecture approaches
- Aligning a team on product direction before writing any code

### Step 2: `/speckit.uidesign` — Visual Design Specification

Takes `product-spec.md` as input, then works with you to define the visual interface. Rather than generate UI directly, it produces **AI-image-generation prompts** for tools like Google Stitch, v0, or Bolt, enabling rapid mockup iteration.

The session covers:
- **Design system** — color palette derivation, typography, component styles
- **Screen-by-screen specs** — layout, navigation, component hierarchy
- **Interaction patterns** — state changes, transitions, error states
- **Responsive strategy** — breakpoints, mobile adaptations

Output: `ui-spec.md` in `.specify/memory/`, containing the full visual design system, screen specifications, and mockup generation prompts ready for AI design tools.

**Use this command when:**
- You need to establish a visual identity before writing backend code
- You want mockup prompts to feed into Stitch, v0, or similar tools
- Aligning frontend and backend decisions early

### Step 3: `/speckit.initiate` — Project Constitution

Synthesizes `product-spec.md` and `ui-spec.md` into a comprehensive `constitution.md`—the project's governing document. This is the **most important step** before any feature development begins.

The constitution captures:
- **Coding standards** — language-specific conventions, style guides
- **Architecture principles** — patterns to follow, anti-patterns to avoid
- **Testing requirements** — coverage thresholds, test types, tooling
- **Security guidelines** — authentication patterns, data handling rules
- **Design token constraints** — colors, spacing, typography to use
- **Dependency decisions** — approved libraries and their rationale

Output: `.specify/memory/constitution.md`, which every subsequent `/speckit.plan` command reads to ensure constitutional compliance.

**Use this command when:**
- Completing initial project setup (run once, update rarely)
- Onboarding a new AI agent to an existing project
- The team needs to revisit and re-establish development standards

### Step 4: `/speckit.specify` — Feature Specification

With the constitution established, you're now ready to use the standard SDD feature workflow. Each feature gets its own specification in `specs/`, branched from `main`.

```bash
/speckit.specify Real-time chat system with message history and user presence
```

This flows into the complete feature-first cycle: `specify → plan → tasks → implement`.

---

### Example: Launching a SaaS Product from Zero

```text
Day 1 — Discovery:
  /speckit.brainstorm
  → Interactive Q&A about your product idea
  → Produces: .specify/memory/product-spec.md

Day 1 — Design:
  /speckit.uidesign
  → Iterates on mockup prompts with you
  → Produces: .specify/memory/ui-spec.md

Day 1 — Constitution:
  /speckit.initiate
  → Synthesizes both specs into governing principles
  → Produces: .specify/memory/constitution.md

Day 2+ — Features (repeat for each one):
  /speckit.specify User authentication with social login
  /speckit.plan
  /speckit.tasks
  /speckit.implement
```

In a single day, you go from a rough idea to a project with complete product requirements, UI design system, and development standards—ready to start feature-driven development on Day 2.
