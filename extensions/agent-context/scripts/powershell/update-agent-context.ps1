...
foreach ($ContextFile in $ContextFiles) {
    ...
    if ($ContextFile.EndsWith(".mdc")) {
        $stripped = $newContent.TrimStart()
        $leading = $newContent.Substring(0, $newContent.Length - $stripped.Length)
        if (-not $stripped.StartsWith("---")) {
            $newContent = "---\nalwaysApply: true\n---\n\n" + $newContent
        } else {
            if ($stripped -match "(?s)^(---[ \t]*`r?`n)(.*?)(`r?`n---[ \t]*)`r?`n([\s\S]*)") {
                $opening = $Matches[1]
                $fmText = $Matches[2]
                $closing = $Matches[3]
                $rest = $Matches[4]
                if (-not $fmText.Contains("alwaysApply:")) {
                    $fmLines = $fmText -split "`r?`n" | Where-Object { $_.Trim() }
                    $fmLines += "alwaysApply: true"
                    $fmText = ($fmLines -join "`n") + "`n"
                } else {
                    $fmLines = $fmText -split "`r?`n" | ForEach-Object {
                        if ($_.Contains("alwaysApply:")) { "alwaysApply: true" } else { $_ }
                    }
                    $fmText = ($fmLines -join "`n") + "`n"
                }
                $newContent = $leading + $opening + $fmText + $closing + "`n" + $rest
            } else {
                $newContent = "---\nalwaysApply: true\n---\n\n" + $newContent
            }
        }
