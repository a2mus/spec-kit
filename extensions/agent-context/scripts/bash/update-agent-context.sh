...
if ctx_path.endswith(".mdc"):
    ...
    leading = new_content[:len(new_content) - len(stripped)]
    if not stripped.startswith("---"):
        ...
        if not match:
            ...
            if "alwaysApply:" not in fm_text:
                fm_lines = [l for l in fm_text.splitlines() if l.strip()]
                fm_lines.append("alwaysApply: true")
                fm_text = "\n".join(fm_lines) + "\n"
            else:
                fm_text = "\n".join("alwaysApply: true" if "alwaysApply:" in l else l for l in fm_text.splitlines()) + "\n"
            new_content = leading + opening + fm_text + closing + sep + rest
