---
description: Brutally honest end-to-end app review — code, UX, security, and product strategy with comprehensive analysis and actionable recommendations.
scripts:
  sh: scripts/bash/check-prerequisites.sh --json --check-code --detect-project-type
  ps: scripts/powershell/check-prerequisites.ps1 -Json -CheckCode -DetectProjectType
handoffs:
  - label: Fix Critical Issues
    agent: speckit.implement
    prompt: Address critical flaws identified in the brutal review. Focus on security vulnerabilities, data-loss risks, and ship-blocking bugs first.
  - label: Create Implementation Plan
    agent: speckit.plan
    prompt: Create a plan to address the improvement proposals from the brutal review.
---
...
Run `{SCRIPT}` from repo root to detect project type and code statistics.
...
- `bin/` or `scripts/` directory with executable files
