...
2. **Validate availability**:
   - If `product-spec.md` is **missing**: ERROR — "Product specification not found. Run `__SPECKIT_COMMAND_BRAINSTORM__` first to define your product."
   - If `ui-spec.md` is **missing**: WARN — "UI specification not found. UI features will be derived from product spec only. Consider running `__SPECKIT_COMMAND_UIDESIGN__` for richer UI decomposition." Proceed if user confirms.
   - If `constitution.md` is **missing** or still contains template placeholders (e.g., `[PRINCIPLE_1_NAME]`): WARN — "Constitution not found or not yet customized. Technical constraints will be inferred from the product spec. Consider running `__SPECKIT_COMMAND_INITIATE__` first." Proceed if user confirms.
...
2. **Generate a manifest file** at `.specify/memory/features/README.md`:
   ...
   **Total Features**: [N]
   ...
   Process features sequentially using `__SPECKIT_COMMAND_SPECIFY__`:

   1. Read the feature brief in each numbered file
   2. Run `__SPECKIT_COMMAND_SPECIFY__ [paste the feature description from the file]`
...
3. **For each feature**, create a file named `NNN-[short-name].md` (zero-padded 3-digit number, kebab-case name) in `.specify/memory/features/`:
   ...
   **Estimated Complexity**: [Small / Medium / Large / XL]
   ...
   [2-4 paragraph natural language description of the feature. This is the text that will be passed directly to `__SPECKIT_COMMAND_SPECIFY__` as the feature description argument.
   ...
   [3-7 high-level acceptance criteria bullets. These will be expanded by `__SPECKIT_COMMAND_SPECIFY__` into full user stories and acceptance scenarios.]
   ...
   **To create the full specification, run:**
   ```
   __SPECKIT_COMMAND_SPECIFY__ [paste the Feature Description section above]
   ```
   ```
...
1. **Start with Feature #001**: `__SPECKIT_COMMAND_SPECIFY__ [description from 001]`
...
- **Feature descriptions are the primary output**: They must be rich enough to be pasted directly into `__SPECKIT_COMMAND_SPECIFY__` and produce a complete spec without further context.
