---
name: speckit-prompt-master
description: Generates optimized prompts for AI tools. Activates only when the user
  explicitly asks to write, fix, improve, or adapt a prompt for a specific AI tool
  (LLM, Cursor, Midjourney, image AI, video AI, coding agents, etc.). Does not activate
  for general conversation, coding tasks, document writing, or other non-prompt-engineering
  work.
compatibility: Works with any spec-kit project. No special directory requirements.
metadata:
  author: github-spec-kit
  source: templates/commands/prompt-master.md
---
...
Identify the tool and route accordingly.
...
Steps:
1. Extract the intent, target tool, constraints, and output format from the pasted prompt
2. Identify what techniques are used (role, CoT, few-shot, etc.)
3. Evaluate what works and what doesn't for the target tool
4. Rebuild or adapt the prompt following the tool-specific routing rules above
...
For prompts targeting agentic tools (Claude Code, Devin, Cursor, Windsurf, Cline, Bolt, SWE-agent, Manus, or anything that executes commands or edits files — mandatory for any prompt referencing filesystem, terminal, dependency, or database operations), append this notice:
